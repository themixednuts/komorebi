...
pub struct FileItem {
    ...
    // komorebi: retain only Windows paths that cannot round-trip through the
    // UTF-8 search projection. Ordinary paths keep the upstream memory shape.
...
impl Clone for FileItem {
    fn clone(&self) -> Self {
        Self {
            ...
            #[cfg(target_os = "windows")]
            exact_path: self.exact_path.clone(),
            size: self.size,
...
impl FileItem {
    ...
    pub fn absolute_path(&self, arena: impl FFFStringStorage, base_path: &Path) -> PathBuf {
        #[cfg(target_os = "windows")]
        if let Some(path) = &self.exact_path {
            return path.clone();
        }
        let mut buf = [0u8; PATH_BUF_SIZE];
    ...
    #[cfg(target_os = "windows")]
    pub(crate) fn retain_exact_path(&mut self, path: &Path) {
        if path.to_str().is_none() {
            self.exact_path = Some(path.to_path_buf());
        }
    }
