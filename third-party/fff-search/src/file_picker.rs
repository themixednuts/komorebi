...
impl FileItem {
    ...
    pub fn new_from_walk_parts(
        ...
        let mut item = Self::new_raw(fname_offset, size, modified, git_status, is_binary);
        #[cfg(target_os = "windows")]
        item.retain_exact_path(path);
        (item, rel_str)
    }
    ...
    pub fn new_from_walk_bytes(
        ...
        let mut item = Self::new_raw(basename_offset, size, modified, git_status, is_binary);
        #[cfg(target_os = "windows")]
        item.retain_exact_path(path);
        (item, decoded)
    }
...
pub struct FilePickerOptions {
    // komorebi: the index root is an authoritative filesystem operand, not
    // presentation text. PathBuf preserves Windows WTF-16 end to end.
...
impl FilePicker {
    ...
    pub fn new(options: FilePickerOptions) -> Result<Self, Error> {
        let path = options.base_path.clone();
        if !path.exists() {
            error!("Base path does not exist: {}", path.display());
