...
/// A lossless, NUL-terminated Windows path for `W` APIs.
...
impl WidePath {
    ...
    /// # Errors
    ///
    /// Returns an error when the path contains an interior NUL, which cannot cross a Win32
    /// pointer-and-terminator boundary without truncation.
...
mod tests {
    ...
    #[test]
    fn preserves_ill_formed_utf16_without_replacement() -> Result<(), WidePathError> {
        let source = [
            u16::from(b'C'),
            u16::from(b':'),
            u16::from(b'\\'),
    ...
    #[test]
    fn preserves_unc_and_verbatim_prefixes() -> Result<(), WidePathError> {
        for source in [
    ...
    #[test]
    fn rejects_interior_nul_instead_of_truncating() {
