...
`std::path::Prefix` is the authoritative syntax classifier on Windows. `normpath` is the focused candidate when a boundary specifically requires a normalized absolute `OsStr`-backed path; normalization is not an identity check or an authorization decision. Existing-path authorization remains handle-based. Do not adopt the similarly named `verbatim` crate: its current implementation panics for UNC, already-verbatim, device-namespace, and empty paths and therefore cannot preserve the required domain. Rust's standard library handles ordinary filesystem calls; a direct Win32 adapter that specifically requires an extended-length spelling must accept only typed disk/UNC inputs and test every prefix class rather than rewriting separators or prefixes generically. The `wtf_string` crate supplies explicit `Wtf8String` and `Wtf16String` types and is the preferred candidate if measurement justifies avoiding repeated wide-string conversion in a hot Win32 adapter; it is not a domain type, and adopting its young API requires its own review and benchmark.
...
- [`normpath` native Windows path normalization](https://docs.rs/normpath/latest/normpath/)
- [`verbatim` source audit showing incomplete prefix handling](https://docs.rs/verbatim/latest/src/verbatim/lib.rs.html)
